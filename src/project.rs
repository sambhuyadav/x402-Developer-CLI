...
"#,
    ...
    pub fn install_dependencies(&self) -> Result<()> {
        match self.framework.as_str() {
            "next" => {
                self.run_npm_command("init -y")?;
                self.run_npm_command(
                    "install next@latest react@latest react-dom@latest @aptos-labs/aptos-ts-sdk",
                )?;
            }
            _ => {
                self.run_npm_command("init -y")?;
                self.run_npm_command(
                    "install react@latest react-dom@latest @aptos-labs/aptos-ts-sdk",
                )?;
            }
        }
